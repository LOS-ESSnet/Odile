...
sparql2 = SPARQLWrapper("http://graphdb.linked-open-statistics.org/repositories/pop5")
...
def population(codecom):
    ...
    sparql.setQuery(query)
    results = sparql.query().convert()
    print(results["results"]["bindings"][0]['nom']['value'])
...
def population_structure(codecom,gender):
    ...
    SELECT ?areacode (sum(?pop) as ?popTotale)  ?tactrNotation ?tactrLabel 
    WHERE    {{
    ...
    print(results)
    return {'x': [result['tactrLabel']['value'] for result in results["results"]["bindings"]],
        'y': [result['popTotale']['value'] for result in results["results"]["bindings"]],
        'type': 'bar',
